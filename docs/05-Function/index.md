...
----
###  数学の関数は引数を〜〜〜して値を導くもの
<small>数学でよくこんなのを見かける</small>  
数学の関数の例）

f(x) = 2x + 5

これは、*とある数 x を”渡す”とxの2倍に5を足した 値を導く関数*である。

この *”渡す”数、つまり()の中の数を、引数(ひきすう)と呼ぶ。*

引数として`2`を渡せば、 `f(2)` は`9`という値を示す。
...
プログラムの場合は *値には型<small>（intとかfloatとか）</small>があるので*、
...
* *`(double x)` の部分では、引数の名前が`x`であること、その型が `double` 型であることを定義している*
 ...
 * <small>つまりreturn が複数書いてあっても関数が返す値は 1 つだけ</small>
...
*プログラムの関数は、返り値を返すこと以外に  
 「副作用を伴う」ことがままある*  
<small>プログラムにおいては、返り値が不要で、副作用だけが必要な場合もある</small>

<small>※副作用：例えばprintf()による出力、exit()によるプログラムの終了等。</small>
