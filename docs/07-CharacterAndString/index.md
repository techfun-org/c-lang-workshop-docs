...
配列の添字に書かなかった場合は、他の配列と同様に初期化する文字列の分だけ配列が確保される。
...
ただし改行が含まれていないので非常に使いにくい。
後述するprintf関数が柔軟で使う安いので、そちらを使うことを勧める。
...
----
### (補足1) printfについて
printf関数は、リテラルを指定したフォーマットで標準出力に書き出すための関数である。
...
上記のコードは、aに入力されるのは先頭の19バイトまでしか入力されず、それ以上の文字列が入力されるとバッファオーバーフローが発生する。
